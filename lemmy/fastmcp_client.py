        ...
        for name, spec in servers.items():
            if isinstance(spec, str):
                ...
                if "://" not in target and " " in target:
                    ...
                    if len(parts) > 1:
                        cfg["args"] = parts[1:]
                    # expand env vars and user home in command and args
                    cmd0 = cfg.get("command")
                    if isinstance(cmd0, str):
                        cfg["command"] = os.path.expandvars(os.path.expanduser(cmd0))
                    args0 = cfg.get("args")
                    if isinstance(args0, list):
                        cfg["args"] = [os.path.expandvars(os.path.expanduser(a)) if isinstance(a, str) else a for a in args0]
                ...
                if isinstance(cmd, str) and "args" not in cfg and " " in cmd:
                    ...
                    if len(parts) > 1:
                        cfg["args"] = parts[1:]
                # expand env vars and user home in command and args
                cmd2 = cfg.get("command")
                if isinstance(cmd2, str):
                    cfg["command"] = os.path.expandvars(os.path.expanduser(cmd2))
                args2 = cfg.get("args")
                if isinstance(args2, list):
                    cfg["args"] = [os.path.expandvars(os.path.expanduser(a)) if isinstance(a, str) else a for a in args2]
                self._servers[name] = cfg
