...
class App:
    def __init__(self) -> None:
        ...
        self.client = OpenAIClient(self.config.api_base)
        ...
        if self.config.models is None:
            ...
            if not self.models and not self.config.default_model:
                print_error(self.console, "No models available from LM Studio API")
                self.models = {}
                self.default_model = ""
                self.model = ""
            else:
        ...
        if self.config.mcp_servers:
            ...
            spinner = Spinner("dots", text="Loading MCP servers...", style="bold medium_purple3")
    ...
    def _load_tools_schema(self, path: str | None = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with resources.files("lemmy.tools").joinpath("schema.json").open("r", encoding="utf-8") as f:
                data = json.load(f)
    ...
    def respond_with_tools(
        ...
        spinner_style: str = "medium_purple3",
    ) -> str:
        """Handle tool-calling loop with LM Studio's chat/completions endpoint and local tool execution.
        ...
        with Live(spinner, console=self.console, refresh_per_second=24, transient=True) as live:
            # Use a working copy of the history so tool call artifacts are not persisted
            working_history: List[Dict[str, Any]] = copy.deepcopy(message)
            try:
            ...
            while iterations < max_iterations:
                ...
                if not tool_calls:
                    break
                # Append assistant tool_calls message to working history only (do not persist)
        ...
        # Always ensure persisted history excludes any tool/tool_calls artifacts
        self.messages[:] = [
            m
            for m in self.messages
            if m.get("role") in ("system", "user", "assistant") and not m.get("tool_calls")
        ]
        # Maintain sliding window after cleanup
                        ...
                        if suppress_until_close:
                            ...
                            if close_idx != -1:
                                ...
                                if to_emit:
                                    ...
                                    live.update(
                                        Markdown(visible_accum, code_theme="dracula", style="medium_purple3"),
                                        refresh=True,
                                    )
    ...
    def reset(self) -> None:
        ...
        try:
            self.set_prompt(
                ...
                spinner_style="bold medium_purple3",
            )
        except Exception as e:
    ...
    def change_model(self, *, reset: bool = False) -> None:
        ...
        self.console.print(f"[bold dodger_blue3]Current model[/]: [bold]{self.model}[/]")
        ...
        self.console.print(
            f"[bold dodger_blue3]Available models[/]: {', '.join(sorted(short_names))}"
        )
